...
if not ((2, 6) <= sys.version_info[:3] < (3,) or (3, 4) <= sys.version_info[:3]):
...
def UsingCrcmodExtension(crcmod_module):
    return getattr(crcmod_module, "crcmod", None) and getattr(
        crcmod_module.crcmod, "_usingExtension", None
    )
