...
class BotoTranslation(CloudApi):
  ...
  def __init__(self,
    ...
    Args:
      ...
      provider: Provider prefix describing cloud storage provider to connect to.
        'gs' and 's3' are supported. Function implementations ignore the
        provider argument and use this one instead.
      credentials: Unused.
  ...
  def _PerformResumableDownload(self,
    ...
    Args:
      ...
      callback: (optional) a callback function that will be called to report
        ...
        parameters.  The first integer represents the number of bytes that have
        been successfully transmitted from the service.  The second represents
        the total number of bytes that need to be transmitted.
      num_callbacks: (optional) If a callback is specified with the callback
        parameter, this determines the granularity of the callback by defining
        the maximum number of times the callback will be called during the file
        transfer.
  ...
  def _StorageUriForObject(self, bucket, object_name, generation=None):
    ...
    Args:
      ...
      generation: Generation or version_id of object.  If None, live version of
        the object is used.
