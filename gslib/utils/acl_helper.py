...
class AclChange(object):
    ...
    def _YieldMatchingEntries(self, current_acl):
        ...
        for entry in current_acl:
            if (
                self.scope_type in ("UserById", "GroupById")
                and entry.entityId
                and self.identifier == entry.entityId
            ):
                yield entry
            elif (
                self.scope_type in ("UserByEmail", "GroupByEmail")
                and entry.email
                and self.identifier == entry.email
            ):
                yield entry
            elif (
                self.scope_type == "GroupByDomain"
                and entry.domain
                and self.identifier == entry.domain
            ):
                yield entry
            elif (
                self.scope_type == "Project"
                and entry.projectTeam
                and self.identifier
                == "%s-%s" % (entry.projectTeam.team, entry.projectTeam.projectNumber)
            ):
                yield entry
            elif (
                self.scope_type == "AllUsers"
                and entry.entity.lower() == self.public_entity_all_users.lower()
            ):
                yield entry
            elif (
                self.scope_type == "AllAuthenticatedUsers"
                and entry.entity.lower() == self.public_entity_all_auth_users.lower()
            ):
                yield entry
    ...
    def _AddEntry(self, current_acl, entry_class):
        ...
        if self.scope_type == "UserById":
            ...
            raise CommandException(
                "Add entry to ACL got unexpected scope type %s." % self.scope_type
            )
    ...
    def Execute(self, storage_url, current_acl, command_name, logger):
        ...
        if self.perm == "WRITER":
            if command_name == "acl" and storage_url.IsObject():
                ...
                raise CommandException(
                    "WRITER cannot be set as a default object ACL "
    ...
    def _YieldMatchingEntries(self, current_acl):
        ...
        for entry in current_acl:
            if entry.entityId and self.identifier.lower() == entry.entityId.lower():
                ...
                entry.entity.lower() == "allauthenticatedusers"
                and self.identifier == "AllAuthenticatedUsers"
            ):
                yield entry
