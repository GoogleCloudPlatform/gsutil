...
CERTIFICATE_VALIDATION_ENABLED = config.get("Boto", "https_validate_certificates", True)
            ...
            if not os.path.exists(configured_certs_file):
                ...
                if not certs_data:
                    raise CommandException(
                        "Certificates file not found. Please "
...
def ConfigureNoOpAuthIfNeeded():
    ...
    if not HasConfiguredCredentials():
        if config.has_option("Credentials", "gs_service_client_id") and not HAS_CRYPTO:
...
def GetJsonResumableChunkSize():
    chunk_size = config.getint(
        "GSUtil", "json_resumable_chunk_size", long(1024 * 1024 * 100)
    )
    if chunk_size == 0:
...
def GetNewHttp(http_class=httplib2.Http, **kwargs):
    ...
    proxy_info = httplib2.ProxyInfo(
        ...
        proxy_rdns=config.get("Boto", "proxy_rdns", True if proxy_host else False),
    )
...
def HasConfiguredCredentials():
    ...
    has_goog_creds = config.has_option(
        "Credentials", "gs_access_key_id"
    ) and config.has_option("Credentials", "gs_secret_access_key")
    has_amzn_creds = config.has_option(
        "Credentials", "aws_access_key_id"
    ) and config.has_option("Credentials", "aws_secret_access_key")
    ...
    if has_goog_creds or has_amzn_creds or has_oauth_creds or has_service_account_creds:
        return True
    ...
    try:
        ...
        if (
            getattr(getattr(valid_auth_handler, "__class__", None), "__name__", None)
            == "NoOpAuth"
        ):
            valid_auth_handler = None
...
def MonkeyPatchBoto():
    ...
    def _PatchedGetPluginMethod(cls, requested_capability=None):
        ...
        new_result = [r for r in handler_subclasses if r not in xml_oauth2_handlers] + [
...
def UsingCrcmodExtension(crcmod):
    return boto.config.get("GSUtil", "test_assume_fast_crcmod", None) or (
        getattr(crcmod, "crcmod", None)
        and getattr(crcmod.crcmod, "_usingExtension", None)
    )
