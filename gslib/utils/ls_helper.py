...
def PrintFullInfoAboutObject(bucket_listing_ref, incl_acl=True):
  ...
  Args:
    bucket_listing_ref: BucketListingRef being listed. Must have ref_type OBJECT
      and a populated root_object with the desired fields.
...
class LsHelper(object):
  ...
  def __init__(self,
    ...
    Args:
      iterator_func: Function for instantiating iterator. Inputs- url_string-
        Url string to iterate on. May include wildcards. all_versions=False- If
        true, iterate over all object versions.
      ...
      print_dir_header_func: Function for printing header line for buckets or
        prefixes.
      print_bucket_header_func: Function for printing header line for buckets or
        prefixes.
      ...
      fields:            Fields to request from bucket listings; this should
        include all fields that need to be populated in objects so they can be
        listed. Can be set to None to retrieve all object fields. Defaults to
        short listing fields.
  ...
  def _RecurseExpandUrlAndPrint(self, url_str, print_initial_newline=True):
    ...
    Args:
      url_str: String describing StorageUrl to iterate over. Must be of depth
        one or higher.
