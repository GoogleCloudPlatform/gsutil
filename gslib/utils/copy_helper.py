...
def _CreateDigestsFromDigesters(digesters):
    ...
    if digesters:
        for alg in digesters:
            digests[alg] = (
                base64.encodestring(digesters[alg].digest())
                .rstrip(b"\n")
                .decode("ascii")
            )
    return digests
...
def _DoSlicedDownload(
    ...
    for cp_result in cp_results:
        ...
        server_gzip = cp_result.server_encoding and cp_result.server_encoding.lower().endswith(
            "gzip"
        )
...
def _ValidateAndCompleteDownload(
    ...
    if digesters_succeeded:
        ...
        local_hashes = _CreateDigestsFromLocalFile(
            gsutil_api.status_queue, hash_algs, file_name, src_url, src_obj_metadata
        )
...
def PerformCopy(
    ...
    if dst_url.scheme == "s3" and src_obj_size > S3_MAX_UPLOAD_SIZE and src_url != "s3":
        raise CommandException(
