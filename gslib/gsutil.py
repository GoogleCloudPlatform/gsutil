...
sys.path.insert(0, gsutil_bin_dir)
...
usage_string = """
SYNOPSIS
  gsutil [-d][-D] [-h header]... [-m] [-n] [command args...]
  ...
  -n option inhibits redacting of user credentials in debugging output.
...
commands = {
    ...
    'cp': [command_inst.CopyObjsCommand, 2, NO_MAX, 'a:eprRtz:', True, False,
           0],
    'disablelogging' : [command_inst.DisableLoggingCommand,
        1, NO_MAX, '', False, False, 0],
    'enablelogging' : [command_inst.EnableLoggingCommand,
        1, NO_MAX, 'b:o:a:', False, False, 0],
    'getacl': [command_inst.GetAclCommand, 1, 1, '', False, False, 0],
    'getdefacl': [command_inst.GetDefAclCommand, 1, 1, '', False, False, 0],
    'getlogging': [command_inst.GetLoggingCommand, 1, 1, '', False, False, 0],
    ...
    'mv': [command_inst.MoveObjsCommand, 2, NO_MAX, 'p', True, False, 0],
    ...
    'setdefacl': [command_inst.SetDefAclCommand, 2, NO_MAX, '', False, False, 
                  1],
...
xml_parsing_commands = ['getacl', 'getdefacl', 'getlogging', 'ls', 
                        'setacl', 'setdefacl']
...
def main():
  ...
  global debug
  global insert_warning
  try:
    opts, args = getopt.getopt(sys.argv[1:], 'dDvh:mn',
                               ['debug', 'detailedDebug', 'version', 'help',
                                'header', 'multithreaded', 'noSanitize'])
    if not args:
      cmd = 'help'
    else:
      cmd = args[0]
    ...
    for o, a in opts:
      if o in ('-n', '--noSanitize'):
        # undocumented option to insert warning when not sanitizing output
        insert_warning = True
      if o in ('-d', '--debug'):
