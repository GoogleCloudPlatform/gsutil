    ...
    def GetConnectionClass(self):
        ...
        class UploadCallbackConnection(httplib2.HTTPSConnectionWithTimeout):
            ...
            def putheader(self, header, *values):
                ...
                # update the modifier.
                if (
                    self.header_encoding == "gzip"
                    and self.header_length
                ...
                while partial_buffer:
                    if six.PY2:
                        ...
                        if isinstance(partial_buffer, bytes):
                            ...
                            httplib2.HTTPSConnectionWithTimeout.send(
                                self, partial_buffer.encode("utf-8")
                            )
    ...
    def GetConnectionClass(self):
        ...
        class DownloadCallbackConnection(httplib2.HTTPSConnectionWithTimeout):
            ...
            def getresponse(self, buffering=False):
                ...
                if orig_response.status not in (
                    http_client.OK,
    ...
    def _conn_request(self, conn, request_uri, method, body, headers):
        ...
        try:
            ...
            if method == "HEAD":
                conn.close()
                response = httplib2.Response(response)
            elif method == "GET" and response.status in (
                http_client.OK,
                http_client.PARTIAL_CONTENT,
            ):
                content_length = None
                ...
                if content_length is not None and long(bytes_read) != long(
                    content_length
                ):
