...
def PatchBindings(base, diff):
  ...
  # Currently, setting allUsers or allAuthenticatedUsers is only possible if no
  # other members are set for the role specified. We are handling that case
  # here.
  #
  # TODO(iam-beta): Delete this block of code (and associated tests) when
  # allUsers / allAuthenticatedUsers can be set in conjunction with other
  # members.
  for binding in bindings:
    public_intersection = PUBLIC_MEMBERS.intersection(binding.members)
    if public_intersection:
      if len(public_intersection) == 1:
        binding.members = list(public_intersection)
      # Both allUsers and allAuthenticatedUsers appear in the list.
      # This should only ever happen if we are granting a new role,
      # as the old role is guaranteed to be correct.
      else:
        public_intersection.intersection_update(tmp_diff[binding.role])
        if len(public_intersection) > 1:
