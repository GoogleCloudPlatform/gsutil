...
def _SetIamWrapper(serialized_policy, cls, expansion_result, thread_state):
...
def _PatchIamWrapper(
    serialized_bindings_tuples, cls, expansion_result, thread_state):
...
class IamCommand(Command):
  ...
  def _PatchIam(self):
    ...
    if threaded_wildcards:
      ...
      # Command.Apply will parallelize object handling if the -m flag is set.
      self.Apply(
          partial(
              _PatchIamWrapper,
              # Serialize the unpickleable objects in preparation for
              # Command.Apply.
  ...
  def _SetIam(self):
    ...
    if threaded_wildcards:
      ...
      self.Apply(
          # Serialize the unpickleable objects in preparation for
          # Command.Apply.
          partial(_SetIamWrapper, protojson.encode_message(policy)),
          name_expansion_iterator,
