...
def _GenerateFileData(fp,
  ...
  Args:
    ...
    max_unique_random_bytes: The maximum number of bytes to generate
      pseudo-randomly before beginning to repeat bytes. The default was chosen
      as the next prime larger than 5 MiB.
  """
  # Normalize the ratio.
  random_ratio /= 100.0
...
class PerfDiagCommand(Command):
  ...
  def PerformFannedDownload(self, need_to_slice, object_names, file_names,
    ...
    Args:
      ...
      serialization_data: A list, corresponding by index to object_names, of
        serialization data for each object.
    """
    args = []
  ...
  def PerformFannedUpload(self,
    ...
    Args:
      need_to_slice: If True, additionally apply the slice strategy to each file
        in file_names.
  ...
  def PerformSlicedUpload(self,
    ...
    Args:
      ...
      gzip_encoded: Flag for if the file will be uploaded with the gzip
        transport encoding. If true, a semaphore is used to limit resource
        usage.
    """
  ...
  def Upload(self,
    ...
    Args:
      ...
      file_start: The first byte in the file to upload to the object. (only
        should be specified for sliced uploads)
      file_size: The size of the file to upload. (only should be specified for
        sliced uploads)
      gzip_encoded: Flag for if the file will be uploaded with the gzip
        transport encoding. If true, a lock is used to limit resource usage.
  ...
  def Download(self,
    ...
    Args:
      ...
      start_byte: The first byte in the object to download. (only should be
        specified for sliced downloads)
      end_byte: The last byte in the object to download. (only should be
        specified for sliced downloads)
    """
    fp = None
    try:
