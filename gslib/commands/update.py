  ...
  def _ExplainIfSudoNeeded(self, tf, dirs_to_remove, old_cwd):
    ...
    Args:
      ...
      old_cwd: Path to the working directory we should chdir back to if sudo is
        needed. It's possible that we've chdir'd to a temp directory that's been
        deleted, which can cause odd behavior (e.g. OSErrors when opening the
        metrics subprocess). If this is not truthy, we won't attempt to chdir
        back to this value.
  ...
  def _CleanUpUpdateCommand(self, tf, dirs_to_remove, old_cwd):
    ...
    Args:
      ...
      old_cwd: Path to the working directory we should chdir back to. It's
        ...
        subprocess). If this is not truthy, we won't attempt to chdir back to
        this value.
    """
    if tf:
      tf.close()
