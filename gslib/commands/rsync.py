...
def _ListUrlRootFunc(cls, args_tuple, thread_state=None):
  ...
  Args:
    ...
    args_tuple: (base_url_str, out_file_name, desc), where base_url_str is
      top-level URL string to list; out_filename is name of file to which sorted
      output should be written; desc is 'source' or 'destination'.
...
class _DiffIterator(object):
  ...
  def _CompareObjects(self, src_url_str, src_size, src_mtime, src_crc32c,
    ...
    use_hashes = (
        self.compute_file_checksums or
  ...
  def __iter__(self):
    ...
    while True:
      ...
      if (dst_url_str is None or src_url_str_to_check < dst_url_str_to_check):
        ...
        should_replace, has_src_mtime, has_dst_mtime = (
            self._CompareObjects(src_url_str, src_size, src_mtime, src_crc32c,
                                 src_md5, dst_url_str, dst_size, dst_mtime,
                                 dst_crc32c, dst_md5))
        if should_replace:
...
class RsyncCommand(Command):
  ...
  def RunCommand(self):
    ...
    is_daisy_chain = (
        src_url.IsCloudUrl() and dst_url.IsCloudUrl() and
