...
def _GenSignedUrl(
    ...
    hashed_canonical_request = base64.b16encode(
        canonical_request_hasher.digest()
    ).lower()
    ...
    signature = base64.b16encode(sign(key, string_to_sign, b"RSA-SHA256")).lower()

    final_url = _SIGNED_URL_FORMAT.format(
        host=gs_host, path=gcs_path, sig=signature, query_string=canonical_query_string
    )
...
def _ReadKeystore(ks_contents, passwd):
    ...
    client_email = (
        ks.get_certificate()
        .get_subject()
        .CN.replace(".apps.googleusercontent.com", "@developer.gserviceaccount.com")
    )
...
class UrlSignCommand(Command):
    ...
    def _ProbeObjectAccessWithClient(self, key, client_email, gcs_path, logger, region):
        ...
        signed_url = _GenSignedUrl(
            key, client_email, "HEAD", timedelta(seconds=60), gcs_path, logger, region
        )
