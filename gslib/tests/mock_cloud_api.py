...
class MockBucket(object):
    ...
    def CreateObject(self, object_name, contents=""):
        return self.CreateObjectWithMetadata(
            MockObject(apitools_messages.Object(name=object_name, contents=contents))
        )
    ...
    def CreateObjectWithMetadata(self, apitools_object, contents=""):
        ...
        if (
            self.root_object.versioning
            and self.root_object.versioning.enabled
            and apitools_object.name in self.objects
        ):
            if "live" in self.objects[object_name]:
                ...
                "versioned" in self.objects[object_name]
                and self.objects[object_name]["versioned"]
            ):
...
class MockCloudApi(object):
    ...
    def MockCreateObjectWithMetadata(self, apitools_object, contents=""):
        ...
        return (
            self.buckets[apitools_object.bucket]
            .CreateObjectWithMetadata(apitools_object, contents=contents)
            .root_object
        )
    ...
    def GetObjectMetadata(
        ...
        if bucket_name in self.buckets:
            ...
            if object_name in bucket.objects and bucket.objects[object_name]:
                if generation:
                    ...
                    if "live" in bucket.objects[object_name]:
                        if (
                            bucket.objects[object_name]["live"].root_object.generation
                            == generation
                        ):
            ...
            raise CreateObjectNotFoundException(
                404, self.provider, bucket_name, object_name
            )
