...
class TestCredsConfig(testcase.GsUtilUnitTestCase):
  ...
  def testMultipleConfiguredCreds(self):
    ...
    try:
      ...
      pass
    config.set('Credentials', 'gs_oauth2_refresh_token', 'foo')
    config.set('GoogleCompute', 'service_account', 'foo')
  ...
  def testExactlyOneInvalid(self):
    ...
    try:
      GcsJsonApi(None, self.logger)
      succeeded = True  # If we self.fail() here, the except below will catch it
