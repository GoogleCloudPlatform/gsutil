...
class TestIamHelpers(testcase.GsUtilUnitTestCase):
  ...
  def test_grant_no_role_error(self):
    ...
    with self.assertRaises(CommandException):
      bstt(True, 'user:foo@bar.com:')
    with self.assertRaises(CommandException):
      bstt(True, 'deleted:user:foo@bar.com:')
  ...
  def test_valid_deleted_member(self):
    """Tests member parsing."""
    (_, bindings) = bstt(True, 'deleted:user:foo@bar.com:admin')
    self.assertEquals(len(bindings), 1)
    self.assertIn(
        bvle(members=['deleted:user:foo@bar.com'], role='roles/storage.admin'),
        bindings)
    ...
    test_policy = {
        'bindings': [{
            ...
            'role': 'roles/storage.admin'
                     ...
                     {
                         'members':
                         ['user:foo@bar.com', 'serviceAccount:bar@foo.com'],
                         'role':
