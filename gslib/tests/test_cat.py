...
class TestCat(testcase.GsUtilIntegrationTestCase):
    ...
    def test_cat_range(self):
        ...
        stderr = self.RunGsUtil(
            ["cat", "-r -", suri(key_uri)], return_stderr=True, expected_status=1
        )
        self.assertIn("Invalid range", stderr)
        stderr = self.RunGsUtil(
            ["cat", "-r a-b", suri(key_uri)], return_stderr=True, expected_status=1
        )
        self.assertIn("Invalid range", stderr)
        stderr = self.RunGsUtil(
            ["cat", "-r 1-2-3", suri(key_uri)], return_stderr=True, expected_status=1
        )
        self.assertIn("Invalid range", stderr)
        stderr = self.RunGsUtil(
            ["cat", "-r 1.7-3", suri(key_uri)], return_stderr=True, expected_status=1
        )
        self.assertIn("Invalid range", stderr)
    ...
    def test_cat_version(self):
        ...
        if RUN_S3_TESTS:
            ...
            stderr = self.RunGsUtil(
                ["cat", uri2.version_specific_uri + "23456"],
                return_stderr=True,
                expected_status=1,
            )
            ...
            stderr = self.RunGsUtil(
                ["cat", uri2.version_specific_uri + "23"],
                return_stderr=True,
                expected_status=1,
            )
            self.assertIn(
                NO_URLS_MATCHED_TARGET % uri2.version_specific_uri + "23", stderr
            )
    ...
    def test_cat_encrypted_object(self):
        ...
        stderr = self.RunGsUtil(
            ["cat", suri(object_uri)], expected_status=1, return_stderr=True
        )
