...
class TestRetentionUtil(testcase.GsUtilUnitTestCase):
    ...
    def testRetentionPeriodToString(self):
        ...
        self.assertRegexpMatches(
            retention_str, r"Duration: 86401 Seconds \(~1 Day\(s\)\)"
        )
        ...
        self.assertRegexpMatches(
            retention_str, r"Duration: 2678399 Seconds \(~30 Day\(s\)\)"
        )

        retention_str = _RetentionPeriodToString(SECONDS_IN_MONTH + 1)
        self.assertRegexpMatches(
            retention_str, r"Duration: 2678401 Seconds \(~31 Day\(s\)\)"
        )
        ...
        self.assertRegexpMatches(
            retention_str, r"Duration: 31557599 Seconds \(~365 Day\(s\)\)"
        )

        retention_str = _RetentionPeriodToString(SECONDS_IN_YEAR + 1)
        self.assertRegexpMatches(
            retention_str, r"Duration: 31557601 Seconds \(~365 Day\(s\)\)"
        )
