...
class TestStat(testcase.GsUtilIntegrationTestCase):
  ...
  def test_stat_output(self):
    ...
    assertIn('Creation time:', stdout)
    assertIn('Cache-Control:', stdout)
    assertIn('Content-Encoding:', stdout)
    assertIn('Content-Length:', stdout)
    assertIn('Content-Type:', stdout)
    assertIn('Hash (crc32c):', stdout)
    assertIn('Hash (md5):', stdout)
