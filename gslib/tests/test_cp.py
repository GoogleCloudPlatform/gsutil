...
def TestCpMvPOSIXBucketToLocalNoErrors(cls, bucket_uri, tmpdir, is_cp=True):
  ...
  cls.VerifyLocalPOSIXPermissions(os.path.join(tmpdir,
                                               'obj1'),
                                  gid=primary_gid,
                                  mode=DEFAULT_MODE)
  cls.VerifyLocalPOSIXPermissions(os.path.join(tmpdir,
    ...
    cls.RunGsUtil(
        ['cp' if is_cp else 'mv',
         ...
         suri(bucket_uri,
              obj_name)])
    if uid != NA_ID:
      cls.VerifyObjectCustomAttribute(bucket_uri.bucket_name,
                                      obj_name,
                                      UID_ATTR,
                                      str(uid))
    if gid != NA_ID:
...
class TestCp(testcase.GsUtilIntegrationTestCase):
  ...
  def _CpWithFifoViaGsUtilAndAppendOutputToList(self,
                                                src_path_tuple,
                                                dst_path,
  ...
  def test_dest_bucket_not_exist(self):
    ...
    invalid_bucket_uri = ('%s://%s' % (self.default_provider,
  ...
  def test_copy_in_cloud_noclobber(self):
    ...
    stderr = self.RunGsUtil(['cp',
                            ...
                            return_stderr=True)
    self.assertIn(
        'Skipping existing item: %s' % suri(bucket2_uri,
                                            key_uri.object_name),
        stderr)
  ...
  def test_detect_content_type(self):
    ...
    def _Check1():
      ...
      if IS_WINDOWS:
        self.assertTrue(
            re.search(r'Content-Type:\s+audio/x-mpg',
                      stdout) or re.search(r'Content-Type:\s+audio/mpeg',
                                           stdout))
      else:
  ...
  def test_versioning_no_parallelism(self):
    ...
    stderr = self.RunGsUtil([
        ...
        'cp',
        '-A',
        suri(self.nonexistent_bucket_name,
             'foo'),
        suri(self.nonexistent_bucket_name,
             'bar')
    ],
  ...
  def test_recursive_copying_versioned_bucket(self):
    ...
    def _Check2():
      ...
      listing1 = self.RunGsUtil(['ls',
                                 '-la',
                                 suri(bucket1_uri)],
                                return_stdout=True).split('\n')
      listing2 = self.RunGsUtil(['ls',
                                 '-la',
                                 suri(bucket2_uri)],
                                return_stdout=True).split('\n')
  ...
  def test_canned_acl_cp(self):
    ...
    # the ACLs.
    self.RunGsUtil(['acl', 'set', 'public-read', suri(key_uri)])
    public_read_acl = self.RunGsUtil(['acl',
                                      'get',
                                      suri(key_uri)],
                                     return_stdout=True)
  ...
  def test_canned_acl_upload(self):
    ...
    new_acl_json = self.RunGsUtil(['acl',
                                   'get',
                                   suri(bucket1_uri,
                                        file_name)],
                                  return_stdout=True)
  ...
  def test_copy_bucket_to_bucket(self):
    ...
    def _CopyAndCheck():
      ...
      stdout = self.RunGsUtil(['ls',
                               '-R',
                               dst_bucket_uri.uri],
                              return_stdout=True)
      self.assertIn(
          '%s%s/obj0\n' % (dst_bucket_uri,
                           src_bucket_uri.bucket_name),
          stdout)
      self.assertIn(
          '%s%s/obj1\n' % (dst_bucket_uri,
                           src_bucket_uri.bucket_name),
  ...
  def test_recursive_download_with_leftover_dir_placeholder(self):
    ...
    dir_list = sorted(dir_list)
    self.assertEqual(len(dir_list), 2)
    self.assertEqual(os.path.join(dst_dir,
                                  src_bucket_uri.bucket_name,
                                  'obj0'),
                     dir_list[0])
    self.assertEqual(os.path.join(dst_dir,
  ...
  def test_cp_md5_match(self):
    ...
    def _Check1():
      ...
      self.assertRegex(
          stdout,
          r'Hash\s+\(md5\):\s+%s' % re.escape(file_md5.decode('ascii')))
    ...
    def _create_test_data():
      ...
      local_uris = [
          self.CreateTempFile(file_name=filename,
                              tmpdir=tmpdir,
                              contents=contents) for filename in ('test.html',
                                                                  'test.js',
                                                                  'test.txt')
      ]
  ...
  def test_cp_resumable_upload_retry(self):
    ...
    if self.test_api == ApiSelector.XML:
      test_callback_file = self.CreateTempFile(contents=pickle.dumps(
          _ResumableUploadRetryHandler(5,
  ...
  def test_cp_resumable_upload_break_file_content_change(self):
    ...
    test_callback_file = self.CreateTempFile(contents=pickle.dumps(
        HaltingCopyCallbackHandler(True,
                                   int(ONE_KIB) * 384)))
    resumable_threshold_for_test = ('GSUtil',
                                    'resumable_threshold',
                                    str(ONE_KIB))
    resumable_chunk_size_for_test = ('GSUtil',
                                     'json_resumable_chunk_size',
                                     str(ONE_KIB * 256))
  ...
  def test_cp_resumable_download_break(self):
    ...
    self._test_cp_resumable_download_break_helper([('GSUtil',
                                                    'resumable_threshold',
                                                    str(ONE_KIB))])
  ...
  def test_cp_resumable_download_content_differs(self):
    ...
    self.assertEqual(len(etag_match.groups()),
                     1,
  ...
  def test_cp_resumable_download_gzip(self):
    ...
    def _GetObjectSize():
      ...
      self.assertEqual(len(size_match.groups()),
                       1,
  ...
  def test_cp_resumable_upload_bucket_deleted(self):
    ...
    boto_config_for_test = ('GSUtil', 'resumable_threshold', str(ONE_KIB))
    test_callback_file = self.CreateTempFile(contents=pickle.dumps(
        _DeleteBucketThenStartOverCopyCallbackHandler(5,
                                                      bucket_uri)))
  ...
  def start_over_error_test_helper(self, http_error_num):
    ...
    if self.test_api == ApiSelector.JSON:
      test_callback_file = self.CreateTempFile(
          contents=pickle.dumps(_JSONForceHTTPErrorCopyCallbackHandler(5,
                                                                       404)))
  ...
  def test_rewrite_cp(self):
    ...
    object_uri = self.CreateObject(bucket_uri=bucket_uri,
                                   object_name='foo',
                                   contents=b'bar')
    gsutil_api = GcsJsonApi(BucketStorageUri,
                            logging.getLogger(),
  ...
  def test_kms_key_correctly_applied_to_dst_obj_from_src_with_no_key(self):
    ...
    with SetBotoConfigForTest([('GSUtil', 'prefer_api', 'json')]):
      self.AssertObjectUsesCMEK('%s/%s' % (suri(bucket_uri),
                                           obj2_name),
                                key_fqn)
...
class TestCpUnitTests(testcase.GsUtilUnitTestCase):
  ...
  def testDownloadWithNoHashAvailable(self):
    ...
    log_handler = self.RunCommand('cp',
                                  [suri(object_uri),
                                   dst_dir],
