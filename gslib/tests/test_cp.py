...
class TestCp(testcase.GsUtilIntegrationTestCase):
    ...
    def test_copy_quiet(self):
        ...
        stderr = self.RunGsUtil(
            ["-q", "cp", suri(key_uri), suri(bucket_uri.clone_replace_name("o2"))],
            return_stderr=True,
        )
    ...
    """
        ...
        with open(fpath, "rb") as f_in:
            file_md5 = base64.encodestring(
                binascii.unhexlify(CalculateMd5FromContents(f_in))
            ).rstrip(b"\n")
    ...
    def test_cp_manifest_upload_unicode(self):
        return self._ManifestUpload("foo-unicöde", "bar-unicöde", "manifest-unicöde")
    ...
    def _ManifestUpload(self, file_name, object_name, manifest_name):
                ...
                if line.startswith("b'"):
                    ...
                    for some_str in some_strs:
                        if some_str.startswith("b'"):
                            line_parts.append(
                                ast.literal_eval(some_str).decode("utf-8")
                            )
                        else:
        ...
        self.assertEqual(results[1][:5], "%s://" % self.default_provider)  # destination
    ...
    """
        ...
        self.assertNotIn(
            "Using compressed transport encoding for file://%s." % (local_uri3), stderr
        )
    ...
    def test_cp_resumable_upload_break_file_content_change(self):
        ...
        test_callback_file = self.CreateTempFile(
            contents=pickle.dumps(HaltingCopyCallbackHandler(True, int(ONE_KIB) * 384))
        )
   ...
   response header).
    ...
    @unittest.skipUnless(UsingCrcmodExtension(crcmod), "Test requires fast crcmod.")
