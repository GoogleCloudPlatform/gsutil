    ...
    if not task_queues:
      ...
      if process_count > 1:
        ...
        WorkerPool(thread_count,
                   ...
                   headers=self.non_metadata_headers, perf_trace_token=self.perf_trace_token, trace_token=self.trace_token,
                   user_project=self.user_project)
...
class WorkerPool(object):
  ...
  def __init__(self,
               ...
               debug=0,
               status_queue=None,
               headers=None, perf_trace_token=None, trace_token=None,
               user_project=None):
    ...
    for _ in range(thread_count):
      worker_thread = WorkerThread(
          ...
          debug=debug,
          status_queue=status_queue,
          headers=self.headers, perf_trace_token=self.perf_trace_token, trace_token=self.trace_token,
          user_project=self.user_project)
  ...
  def __init__(self,
    ...
    if bucket_storage_uri_class and gsutil_api_map:
      self.thread_gsutil_api = CloudApiDelegator(bucket_storage_uri_class,
                                                 ...
                                                 status_queue,
                                                 debug=debug,
                                                 http_headers=self.headers, perf_trace_token=self.perf_trace_token, trace_token=self.trace_token,
                                                 user_project=self.user_project)
