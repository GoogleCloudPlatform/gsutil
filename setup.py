...
def PlaceNeededFiles(self, target_dir):
    ...
    mock_storage_src = (
        mock_storage_src1 if os.path.isfile(mock_storage_src1) else mock_storage_src2
    )
    if not os.path.isfile(mock_storage_src):
        raise Exception(
            "Unable to find required boto test source file at %s or %s."
...
setup(
    ...
    entry_points={"console_scripts": ["gsutil = gslib.__main__:main"]},
